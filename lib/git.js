 ...
 */
export async function getCommits(from, to, execaOptions, gitLogOptions) {
  const { paths, flags } = gitLogOptions || {};
  return (
      ...
      gitLogParser.parse(
        {
          _: [
            `${from ? from + ".." : ""}${to}`,
            ...(flags?.length ? [flags].flat() : []),
            ...(paths?.length ? ["--", ...[paths].flat()] : []),
          ],
        },
  ...
  try {
    ...
    return merge(
      JSON.parse(
        ...
        (await execa("git", ["notes", "--ref", GIT_NOTE_REF, "show", commit], execaOptions).catch(handleError)).stdout
      ),
      JSON.parse(
        (await execa("git", ["notes", "--ref", `${GIT_NOTE_REF}-${ref}`, "show", commit], execaOptions).catch(handleError))
          .stdout
      )
    );
  } catch (error) {
