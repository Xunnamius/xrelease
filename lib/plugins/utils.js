...
export async function loadPlugin({ cwd }, name, pluginsPath) {
  ...
  const file = resolveFrom.silent(basePath, name) || resolveFrom.silent(cwd, name) || resolveFrom.silent(__dirname, name);
  const { default: cjsExport, ...esmNamedExports } = await import(file ? `file://${file}` : name);
